...
{- |
   Module      : CopyAttoparsecState
   Description : Copying attoparsec for fun and profit
   Copyright   : (c) Ivan Lazar Miljenovic
   License     : 3-Clause BSD-style
   Maintainer  : Ivan.Miljenovic@gmail.com

   A variant of "CopyAttoparsec" with an added (useless) state parameter
   to model passing an extra state variable around.

 -}
module CopyAttoparsecState where
...
type Failure' t   r = Input' t -> Added' t -> More -> State -> [String] -> String
                   -> IResult' t r
type Success' t a r = Input' t -> Added' t -> More -> State -> a -> IResult' t r
...
addS :: (Monoid t) =>
        Input' t -> Added' t -> More -> State
     -> Input' t -> Added' t -> More -> State
     -> (Input' t -> Added' t -> More -> State -> r) -> r
addS i0 a0 m0 s0 _i1 a1 m1 _s1 f =
    let !i = i0 <> I (unA a1)
        a  = a0 <> a1
        !m = m0 <> m1
    in f i a m s0
...
bindP m g =
    Parser' $ \i0 a0 m0 st kf ks -> runParser' m i0 a0 m0 st kf $
                                   \i1 a1 m1 _st1 a -> runParser' (g a) i1 a1 m1 st kf ks
{-# INLINE bindP #-}

returnP :: a -> Parser' t a
returnP a = Parser' (\i0 a0 m0 st _kf ks -> ks i0 a0 m0 st a)
{-# INLINE returnP #-}
...
noAdds :: (Monoid t) =>
          Input' t -> Added' t -> More -> State
       -> (Input' t -> Added' t -> More -> State -> r) -> r
noAdds i0 _a0 m0 st f = f i0 mempty m0 st
{-# INLINE noAdds #-}

plus :: (Monoid t) => Parser' t a -> Parser' t a -> Parser' t a
plus a b = Parser' $ \i0 a0 m0 st0 kf ks ->
           let kf' i1 a1 m1 st1 _ _ = addS i0 a0 m0 st0 i1 a1 m1 st1 $
                                      \ i2 a2 m2 st2 -> runParser' b i2 a2 m2 st2 kf ks
               ks' i1 a1 m1 = ks i1 (a0 <> a1) m1
           in  noAdds i0 a0 m0 st0 $ \i2 a2 m2 st2 -> runParser' a i2 a2 m2 st2 kf' ks'
{-# INLINE plus #-}
...
fmapP p m = Parser' $ \i0 a0 m0 st f k ->
            runParser' m i0 a0 m0 st f $ \i1 a1 s1 st1 a -> k i1 a1 s1 st1 (p a)
{-# INLINE fmapP #-}
...
failDesc err = Parser' (\i0 a0 m0 st kf _ks -> kf i0 a0 m0 st [] msg)
...
ensure' :: Int -> Input -> Added -> More -> State -> Failure r -> Success B.ByteString r
        -> IResult' B.ByteString r
ensure' !n0 i0 a0 m0 st0 kf0 ks0 =
    runParser' (demandInput >> go n0) i0 a0 m0 st0 kf0 ks0
  where
    go !n = Parser' $ \i a m st kf ks ->
        if B.length (unI i) >= n
        then ks i a m st (unI i)
        else runParser' (demandInput >> go n) i a m st kf ks

-- | If at least @n@ bytes of input are available, return the current
-- input, otherwise fail.
ensure :: Int -> Parser B.ByteString
ensure !n = Parser' $ \i0 a0 m0 st0 kf ks ->
    if B.length (unI i0) >= n
    then ks i0 a0 m0 st0 (unI i0)
    -- The uncommon case is kept out-of-line to reduce code size:
    else ensure' n i0 a0 m0 st0 kf ks
-- Non-recursive so the bounds check can be inlined:
{-# INLINE ensure #-}

-- | Ask for input.  If we receive any, pass it to a success
-- continuation, otherwise to a failure continuation.
prompt :: Input -> Added -> More -> State
       -> (Input -> Added -> More -> State -> Result r)
       -> (Input -> Added -> More -> State -> Result r)
       -> Result r
prompt i0 a0 _m0 st kf ks = Partial $ \s ->
    if B.null s
    then kf i0 a0 Complete st
    else ks (i0 <> I s) (a0 <> A s) Incomplete st

-- | Immediately demand more input via a 'Partial' continuation
-- result.
demandInput :: Parser ()
demandInput = Parser' $ \i0 a0 m0 st0 kf ks ->
    if m0 == Complete
    then kf i0 a0 m0 st0 ["demandInput"] "not enough bytes"
    else let kf' i a m st = kf i a m st ["demandInput"] "not enough bytes"
             ks' i a m st = ks i a m st ()
         in prompt i0 a0 m0 st0 kf' ks'

-- | This parser always succeeds.  It returns 'True' if any input is
-- available either immediately or on demand, and 'False' if the end
-- of all input has been reached.
wantInput :: Parser Bool
wantInput = Parser' $ \i0 a0 m0 st0 _kf ks ->
  case () of
    _ | not (B.null (unI i0)) -> ks i0 a0 m0 st0 True
      | m0 == Complete  -> ks i0 a0 m0 st0 False
      | otherwise       -> let kf' i a m st = ks i a m st False
                               ks' i a m st = ks i a m st True
                           in prompt i0 a0 m0 st0 kf' ks'

get :: Parser B.ByteString
get  = Parser' $ \i0 a0 m0 st0 _kf ks -> ks i0 a0 m0 st0 (unI i0)

put :: B.ByteString -> Parser ()
put s = Parser' $ \_i0 a0 m0 st0 _kf ks -> ks (I s) a0 m0 st0 ()
...
failK :: Failure a
failK i0 _a0 _m0 _st stack msg = Fail (unI i0) stack msg
{-# INLINE failK #-}

-- | Terminal success continuation.
successK :: Success a a
successK i0 _a0 _m0 _st a = Done (unI i0) a
{-# INLINE successK #-}
